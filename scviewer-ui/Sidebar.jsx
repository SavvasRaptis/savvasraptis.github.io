  ...
  return (
    <aside className="sidebar">
      ...
      <section className="sb-section">
        ...
        <div className="seg">
          <span className="seg-label">Frame</span>
          <div className="seg-group">
            <button className={frame==='GSE'?'on':''} onClick={() => setFrame('GSE')}>GSE</button>
            <button className={frame==='GSM'?'on':''} onClick={() => setFrame('GSM')}>GSM</button>
            <button className={frame==='SM'?'on':''} onClick={() => setFrame('SM')}>SM</button>
          </div>
        </div>
      </section>
      ...
      <section className="sb-section">
        ...
        <button className="dl-btn wide" onClick={onDownloadTrajectory} title="Full trajectory at chosen cadence">
          ...
          <span className="dl-sub">{cadence === 'ten_min' ? '10-minute' : 'hourly'} samples · {frame}</span>
        </button>
        <button className="dl-btn wide year" onClick={onDownloadYear} title={`Download all positions for ${new Date(startISO).getUTCFullYear()} at chosen cadence`}>
          ...
          <span className="dl-sub">{cadence === 'ten_min' ? '~52,560' : '~8,760'} samples per spacecraft · {frame}</span>
        </button>
      </section>
                  ...
                  {list.map(sc => (
                    <li key={sc.id} className={sc.id === selectedScId ? 'focused' : ''}>
                      <label>
                        ...
                        >{sc.name}</span>
                        <span className="sc-era">{SC_AVAILABILITY[sc.id] || ''}</span>
                      </label>
                    </li>
                  ))}
                </ul>
